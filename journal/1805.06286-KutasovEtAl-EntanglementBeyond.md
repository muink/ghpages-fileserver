...
This lower bound agrees with Kutasov's result. However, there are some issues:

- The length integration does not agree with Kutasov's (3.3); the numerator is missing, but it's lower bound agrees. Maybe it's a re-parametrization thing. TODO: match the metric conventions explicitly.

- What is the string deformation parameter $\lambda$? How to translate this to Silverstein's convention $\sim \sqrt{c\lambda_S}$?
