...
- **Different dual geometry:** $\mcal{M}_3\times \mcal{N}$ , AdS now sits in the _interior_ while near the boundary we have a linear dilaton background. In other words,
...
Cutoff-independent C-function (per Kutasov's convention; Silverstein does not have this extra pre-factor of 3):
...
- $\pdv{\,\log Z}{L}$ is related to $T^a_a$, given in (2.3); with Kutasov's convention, we get:
  ...
  > **Conventions:**
  >
  > - [1806.07444] Donnelly & Shyam: $L = 8\pi r^2$
...
- Compare (cf. PengXiang's note):
  ...
  $L_{\min}$ produced from holography (RT) by Kutasov et al. Here $k$ is the level of the _worldsheet_ $\mathrm{SL}(2,\mathbb{R})$ current algebra.
...
- A convenient substitution for the integral:
  ...
  **Note:** results might differ by a $L$-independent "constant".
