...
- The general formula for $L$ agrees with Kutasov's (3.3), with $2\lambda r_0^2 \leftrightarrow  kU_0^2$, and $\ell = 1$. **TODO**: match the metric conventions explicitly.

- With $\lambda = 1/2$, if we restore:
  ...
  We recover Kutasov's result: $L_{\min} = \frac{\pi}{2} \sqrt{k\alpha'}$. Why set $\lambda = 1/2$ ? Because with this choice the boundary (string-frame) metric has a trivial overall factor:

  $$
    \dd{s}^2
    \sim 1\cdot \dd{u}\,\dd{v}
  $$
...
- With $\ell = 1$, $\lambda = k\mu$, and we have:
  ...
  Indeed the two lower bounds do NOT agree with each other. This implies that we need to modify the RT proposal, at least in the UV ($\rho \to 0$) region, since the UV behavior determines the lower bound of the entanglement region.
