...
This is a variable that specifies the boundary theory; in fact, **we've found the _ensemble of theories_**: it is a collection of 1D TQFTs with various (integer) dimensions.
...
Each connected component contributes a factor of $\lambda$. What is $\lambda$ exactly? We can attach any number of handles on each connected component. Summing over the number of handles gives an effective coupling of $\lambda$; this is in fact included in (3.16) but suppressed in the diagrams.
